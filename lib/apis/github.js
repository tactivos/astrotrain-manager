...
module.exports.updateManifest = async function updateManifest({
  ...
  const { data: tree } = await github.gitdata.getTree({
    owner,
    repo: PLATFORM_REPO,
    sha: branch
  })
  ...
  const { data: updatedTree } = await github.gitdata.createTree({
    base_tree: tree.sha,
    owner,
    repo: PLATFORM_REPO,
    tree: [{
  ...
  const { data: commit } = await github.gitdata.createCommit({
  ...
  const { data: updatedRef } = await github.gitdata.updateReference({
    owner,
    ref: `heads/${branch}`,
    repo: PLATFORM_REPO,
    sha: commit.sha
