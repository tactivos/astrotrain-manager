...
module.exports = async function trainManager(payload) {
  ...
  try {
    ...
    if (base !== config.platform.branch.stable) {
      ...
      log(`Updating manifest file for ${base} on ${repository.name}`)
      ...
      const existingPr = await ghApi.getPlatformPR({ github, base, owner })
...
function getNextBase(base) {
  const branches = config.platform.backmerge
  return Object.keys(branches).find(key => branches[key] === base)
}
