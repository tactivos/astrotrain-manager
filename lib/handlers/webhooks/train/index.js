...
module.exports = async function trainManager(payload) {
  ...
  try {
    ...
    await Promise.all(updatableRepos.map(async c => {
    ...
    const { changelogUrl } = await changelog.update({ github, owner, branch: base })
    log(`Updated changelog on ${base}, check it here ${changelogUrl}!`)

    // only create platform repo PR if we're not updating stable
    if (base !== config.platform.branch.stable) {

      const existingPr = await ghApi.getPlatformPR({ github, head: base, owner })

      const nextBase = ghApi.getNextBase(base)

      if (existingPr) {
        log(`Updating ${repository.name} pull request from ${base} to ${nextBase}`)
      } else {
        log(`Creating ${repository.name} pull request from ${base} to ${nextBase}`)
      }
    ...
    const notifications = buildTrainUpdateFailureNotification({ base, head, error })
