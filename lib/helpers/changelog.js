...
exports.update = async function update({
  ...
  if (isValidChangelog(currentChangelog)) {
...
async function getPRsFromCommits({
  ...
  const prs = commitSearch.filter(pr => pr && pr.repository_url)
...
function splitNewestChangesFromOldest(changelog = '') {
  ...
  const newestChanges = changelog.substring(0, indexOfFirstDashes + dashes.length)
  const oldestChanges = changelog.substring(indexOfFirstDashes + dashes.length)
